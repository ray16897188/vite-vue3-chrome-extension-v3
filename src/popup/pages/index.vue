<template>
  ...
  <div class="text-center m-4 flex flex-col">
      ...
      ws.onmessage = (e) => {
        ...
        if (msg.method == 'subscription') {
          if (msg.params.channel == 'deribit_price_index.btc_usd') {
            ...
            if (msg.params.channel == 'book.BTC-PERPETUAL.agg2') {
              // console.log(msg.params.data)
            }
            // console.log(msg)
          ...
          // console.log(msg);
          this.refreshToken = msg.result.refresh_token;
        } else if (msg.id == this.positionsReqId) {
          console.log(msg);
        }
      };
      ws.onopen = () => {
        ws.send(this.heartBeatMsg());
        this.periodTask(() => {
          ws.send(this.pingMsg());
        }, 30000);
        ws.send(this.authMsg());
        ws.send(this.subIndexPriceMsg(this.currencies));
